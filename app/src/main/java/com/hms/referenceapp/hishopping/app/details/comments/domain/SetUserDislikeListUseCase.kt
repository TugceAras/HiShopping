...
class SetUserDislikeListUseCase @Inject constructor(private val commentsRepositoryApp: CommentsRepositoryApp) :
    ...
    override suspend fun execute(params: List<Int>?): Flow<ResultData<Unit>> {
        params?.let {
            if (it.size > 1) {
                return commentsRepositoryApp.setUsersDislikeList(params[0], params[1])
            }
        }
        return commentsRepositoryApp.setUsersDislikeList(0, 0)
    }
}
