...
class CategoryProductsDataSource @Inject constructor(private val productRepository: ProductRepository) :
    BaseResultFlowAsyncDataSource<List<@JvmSuppressWildcards Product>, CategoryProductsRequest> {
    override suspend fun getResult(params: CategoryProductsRequest?): Flow<ResultData<List<Product>>> {
        return productRepository.loadAllByCategory(params!!.keywords)
    }
}