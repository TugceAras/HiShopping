...
class DiscountProductsRepositoryImpl @Inject constructor(
    private val discountProductsDataSource:
    BaseResultFlowAsyncDataSource<@JvmSuppressWildcards List<Product>, DiscountProductsRequest>
) :
    DiscountProductsRepository {
    override suspend fun getDiscountProducts(discountProductsRequest: DiscountProductsRequest):
            Flow<ResultData<List<Product>>> {
        return discountProductsDataSource.getResult(discountProductsRequest)
    }
}