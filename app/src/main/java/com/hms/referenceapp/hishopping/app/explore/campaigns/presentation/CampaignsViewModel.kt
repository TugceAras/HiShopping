...
import com.hms.referenceapp.hishopping.app.explore.campaigns.domain.CampaignsUseCase
...
class CampaignsViewModel  @Inject constructor(
    @CampaignsUseCase private val getCampaignsUseCase:
    BaseResultFlowAsyncUseCase<@JvmSuppressWildcards List<@JvmSuppressWildcards Campaign>, Unit>,
) : BaseViewModel() {

    fun fetchCampaigns()  = liveData {
        emit(ResultData.Loading())
        getCampaignsUseCase.execute().collect {
            emit(it)
        }
    }
}