...
class CategoryProductsRepositoryImpl @Inject constructor(
    private val categoryProductsDataSource:
    BaseResultFlowAsyncDataSource<@JvmSuppressWildcards List<Product>, CategoryProductsRequest>
) :
    CategoryProductsRepository {
    override suspend fun getDiscountProducts(categoryProductsRequest: CategoryProductsRequest):
            Flow<ResultData<List<Product>>> {
        return categoryProductsDataSource.getResult(categoryProductsRequest)
    }
}