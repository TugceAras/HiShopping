...
class GetCategoryProductsUseCase @Inject constructor(private val categoryProductsRepository: CategoryProductsRepository) :
    BaseResultFlowAsyncUseCase<List<@JvmSuppressWildcards Product>, CategoryProductsRequest> {

    override suspend fun execute(params: CategoryProductsRequest?): Flow<ResultData<List<Product>>> {
        return categoryProductsRepository.getDiscountProducts(params!!)
    }
}