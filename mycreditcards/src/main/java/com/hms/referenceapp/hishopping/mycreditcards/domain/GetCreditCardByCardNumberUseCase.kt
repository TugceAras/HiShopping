...
class GetCreditCardByCardNumberUseCase @Inject constructor(private val creditCardRepository: CreditCardRepository) :
    BaseResultFlowAsyncUseCase<CreditCard?, String> {
    override suspend fun execute(params: String?): Flow<ResultData<CreditCard?>> =
        creditCardRepository.getCreditCardByCardNumber(params!!)
}